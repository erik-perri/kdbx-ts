...
import compressInnerData from './compressInnerData';

describe('compressInnerData', () => {
  it.each([
    [
      'none',
      {
    ...
    [
      'gzip',
      {
        ...
        data: Uint8Array.from([0x01, 0x02, 0x03, 0x01, 0x02, 0x03]),
    ...
    'should compress data using the algorithm %s',
    (_, { algorithm, data, expected }) => {
      // Arrange
      // Nothing to arrange.

      // Act
      const result = compressInnerData(algorithm, data);
